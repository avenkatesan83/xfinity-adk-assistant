...
async def handle_user_query(session_id: str, prompt: str) -> str:
    """
    Handles a user query request
    """
    try:
        print("\n🚀 Handling prompt with ADK Agent..."
              f"\nPrompt: {prompt}")
        
        # The global_runner type hint ensures correct methods are used
        #runner: InMemoryRunner = globals.global_runner 
        #response_text = await runner.run_debug(request.prompt)
        if prompt and prompt == globals.welcome_event:
            prompt = globals.default_user_prompt
